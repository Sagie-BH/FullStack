...
const calculator = () => {
    ...
    const operatorElements = Array.from(document.getElementsByClassName('calc-btn-operator')).filter(btn => btn.id !== 'calc-equals');
    const resultArray = [];
    let resetCalc = false;
    ...
    const logDisplayValue = () => {
        // console.log(`Display Value (Arrow): ${display.innerHTML}`);
